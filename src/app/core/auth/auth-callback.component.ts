...
@Component({
  ...
  template: `
    @if (callbackError()) {
      <div class="flex flex-column align-items-center justify-content-center" style="height:100vh; gap:1rem">
        <p class="text-red-500">Sign-in failed: {{ callbackError() }}</p>
        <p class="text-sm text-color-secondary">Check the browser console and Auth0 Dashboard logs for details.</p>
  ...
  ngOnInit(): void {
    ...
    this.auth0.isLoading$.pipe(
      ...
      if (isAuthenticated) {
        ...
        // Check the browser console / Auth0 Dashboard logs for the cause.
