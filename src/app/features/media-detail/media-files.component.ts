          ...
          @for (file of files(); track file.id) {
            <li class="media-files__item">
              <div class="media-files__path">
                ...
                <span class="media-files__path-text" [title]="file.filePath">{{ file.filePath }}</span>
              </div>
              <div class="media-files__meta">
                @if (file.format) {
                  <p-tag [value]="file.format.toUpperCase()" severity="secondary" styleClass="text-xs" />
                }
                @if (file.resolution) {
                  <p-tag [value]="file.resolution" severity="info" styleClass="text-xs" />
                }
                <span class="text-sm text-color-secondary">{{ file.fileSizeBytes | fileSize }}</span>
                <p-button
                  icon="pi pi-copy"
                  size="small"
                  variant="text"
